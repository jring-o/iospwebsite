...
// Heartbeat ingest for the /datanetwork dashboard. Called by the workshop's
// observer node (node-00) every ~5 minutes with the latest gossip samples.
// Guarded by a bearer token (DATANETWORK_PUSH_TOKEN); writes with the secret
// key because the table has no public write policies.
...
export async function POST(request: Request) {
  ...
  const { error, count } = await supabase
    ...
    .upsert(parsed.data, {
      onConflict: 'scraped_at,cluster,peer,metric',
